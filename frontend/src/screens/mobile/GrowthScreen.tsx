  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: T.paper }}>
      <div style={{ padding: 'calc(14px + var(--sat)) 20px calc(24px + var(--sab))' }}>
        ...
        {/* season hero */}
        <div style={{ marginTop: 18, background: T.canopy, borderRadius: 24, padding: '20px 22px', color: T.onDark, position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'relative' }}>
            ...
            <div ref={containerRef} style={{ marginTop: 8, color: '#cfe0c6', width: '100%' }}>
              {diffTrend.length >= 2 && <Sparkline data={diffTrend} width={containerWidth} height={50} color="#A9C2A1" />}
            </div>
          </div>
        </div>
        ...
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ background: T.card, borderRadius: 18, boxShadow: '0 2px 6px rgba(30,42,34,.06)', overflow: 'hidden' }}>
            {plants.map((p, i) => {
              const delta = p.growth && p.growth.length >= 2
                ? p.growth[p.growth.length - 1] - p.growth[0]
                : null;
              return (
                <div key={p.id} onClick={() => onOpen(p)} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontFamily: T.mono, fontSize: 12, color: T.fern, marginTop: 1 }}>
                      {delta !== null ? `+${formatLength(delta, units)}` : 'No data yet'}
                    </div>
                  </div>
                  {p.growth && p.growth.length >= 2 && <Sparkline data={p.growth} width={84} height={34} />}
                </div>
              );
            })}
          </div>
